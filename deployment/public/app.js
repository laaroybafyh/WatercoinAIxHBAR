...
function assessWaterQuality() {
    ...
    if (overallSafe) {
        safeSet(statusIndicator, { text: 'Air Minum Aman', className: 'status-indicator status-safe' });
    } else {
        let warningMessage = 'Air Minum Buruk (';
        if (!isMicrobiologicalSafe) {
            warningMessage += 'Aktifkan UV, ';
        }
        warningMessage += 'Periksa Filter)';
...
function updateParameterPanel() {
    ...
    safeSet(document.getElementById('param-odor'), { text: 'Tidak Berbau', className: 'parameter-value value-safe' });
    safeSet(document.getElementById('param-taste'), { text: 'Tidak Berasa', className: 'parameter-value value-safe' });
